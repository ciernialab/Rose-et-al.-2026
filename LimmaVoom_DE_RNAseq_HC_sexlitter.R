...
FCsamples <- Info3 %>% filter(Brain.Region == "Hippocampus")

FCsamples$Group <- gsub("_Hippocampus","",FCsamples$Group)
...
#https://support.bioconductor.org/p/131179/
#The function is analogous to calling voom followed by duplicateCorrelation and 
#lmFit except for the modified residual df values and residual standard deviation 
#sigma values. This function returns df.residual values that are less than or equal 
#to those from lmFit and sigma values that are greater than or equal to those from 
#lmFit. voomLmFit is more robust to zero counts than calling voom, 
#duplicateCorrelation and lmFit separately and provides more rigorous error 
#rate control.
...
#######################################################################
#plot only M_Saline_salvsTreg
#######################################################################
i="M_Saline_salvsTreg"
tmp=topTreat(tfit,coef=i,number=nrowkeep,adjust.method="fdr")
#nrow(tmp[(tmp$adj.P.Val<0.05),]) # number of DE genes
...
#volcano plot
pdf(file = paste(comp,"_VolcanoHC.pdf", sep=""), wi = 6, he = 6, useDingbats=F)
...
with(subset(genenames, adj.P.Val<0.05 & abs(logFC)>.3), textxy(logFC, -log10(adj.P.Val), labs=mgi_symbol, cex=.6))
...
#male only
list1.up <- which(dt[,c("M_Saline_salvsTreg")]==1)
list1.up <- unique(tfit$genes$mgi_symbol[list1.up])

list1.down <- which(dt[,c("M_Saline_salvsTreg")]==-1)
...
names(x2) <- c("DamSaline_MalePupSaline>Treg", "DamSaline_MalePupSaline<Treg")
...
DEGs_nonsex1 <- x2$`DamSaline_MalePupSaline>Treg`

DEGs_nonsex2 <-x2$`DamSaline_MalePupSaline<Treg`

tmp1 <- subset(GeneSummary2, mgi_symbol %in% DEGs_nonsex1)  
tmp1$DEG <- c("DamSaline_MalePupSaline>Treg")

tmp2 <- subset(GeneSummary2, mgi_symbol %in% DEGs_nonsex2)  
tmp2$DEG <- c("DamSaline_MalePupSaline<Treg")
