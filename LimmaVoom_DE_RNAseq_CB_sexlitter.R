...
##########################################################################################
# 
# Info <- read.csv("/Users/aciernia/Sync/collaborations/Ashwood/MIATcellRNAseq/Sample\ IDs\ and\ Variables\ for\ MIA\ Brain\ Batch-Tag\ Sequencing.csv")
# 
# #remove PAH145H51
# Info <- Info %>% filter(Sample.ID != "PA145H51")
# 
# unique(Info$Pup.Treatment.Group)
# 
# Info$Pup.Treatment.Group <- gsub("Tregs","Treg",Info$Pup.Treatment.Group)
# 
# allsamples <- table(Info$Dam.Treatment.Group,Info$Pup.Treatment.Group,Info$Brain.Region)
# 
# input.files$Sample.ID <- as.character(input.files$Sample.ID)
# Info$Sample.ID <- as.character(Info$Sample.ID)
# 
# #check all samples match
# #Info2 <- merge(Info,input.files, by="Sample.ID", all=T)
# 
# #PA170C59 should actually be PA170FC59
# Info$Sample.ID <- gsub("PA170C59","PA170FC59",Info$Sample.ID)
# 
# #check all samples match
# Info2 <- merge(Info,input.files, by="Sample.ID", all=T)
# 
# 
# table(Info2$Sample.ID %in% input.files$Sample.ID)
# table(input.files$Sample.ID %in% Info2$Sample.ID)
# 
# input.files$Sample.ID[!(Info2$Sample.ID %in% input.files$Sample.ID)]
# Info2$Sample.ID[!(Info2$Sample.ID %in% input.files$Sample.ID)]
# 
# input.files$Sample.ID[!(input.files$Sample.ID %in% Info2$Sample.ID)]
# Info2$Sample.ID[!(input.files$Sample.ID %in% Info2$Sample.ID)]
# 
# 
# #read in litter info 
# litter <- read.csv("/Users/aciernia/Sync/collaborations/Ashwood/MIATcellRNAseq/RNASeq_Pup-Dam\ Litter\ Matching.csv")
# 
# litter <- litter %>% dplyr::select(-Group) %>% distinct()
# 
# Info3 <- merge(Info2,litter, by.x=c("Mouse.Number"), by.y= c("Mouse"), all.x=T)
# 
# #define groups
# Info3$Group <- paste(Info3$Dam.Treatment.Group,Info3$Pup.Treatment.Group, Info3$Sex.x, Info3$Brain.Region, sep="_")
# 
# 
# table(is.na(Info3))
# 
# #write to file
# write.csv(Info3,"MasterExperimentInfo.csv")

Info3 <- read.csv("/Users/aciernia/Sync/collaborations/Ashwood/MIATcellRNAseq/MasterExperimentInfo.csv")
##########################################################################################
#filter for only CB samples
CBsamples <- Info3 %>% filter(Brain.Region == "Cerebellum")


table(CBsamples$Group, CBsamples$Dam..)
##########################################################################################
# Make DEGList
...
RG <- readDGE(CBsamples$filenames, group=CBsamples$Group)
...
setwd(path)
save(RG,Info3, file="RNAseqDEGlist.CB.RData")
...
###########################################################
allsamples <- table(CBsamples$Dam.Treatment.Group,CBsamples$Pup.Treatment.Group,CBsamples$Sex.x)
allsamples

#smallest group is n=7
keep <- rowSums(cpm(RG)>1)>=7
RGcpm <- RG[keep,]
dim(RGcpm)
# 18719    60

#save
save(RG,RGcpm,file="RNAseqDEGlist.CB.RData")
###########################################################
#filtering plot
###########################################################
library(RColorBrewer)
...
#make design matrix
CBsamples$Sex <- factor(CBsamples$Sex.x, levels=c("M","F"))

CBsamples$Dam.Treatment.Group <- factor(CBsamples$Dam.Treatment.Group,levels = c("Saline","PolyIC"))

CBsamples$Pup.Treatment.Group <- factor(CBsamples$Pup.Treatment.Group,levels = c("Saline","Treg"))

CBsamples$DamID <- factor(CBsamples$Dam..)

#dam treatment, pup treatment, and sex
CBsamples$Group <- paste(CBsamples$Dam.Treatment.Group,CBsamples$Pup.Treatment.Group,CBsamples$Sex, sep="_")
table(CBsamples$Group)

#set design matrix: full sex * group interactions
design <- model.matrix(~0+Group, data=CBsamples) #if using a 0 intercept must set up contrasts
...
#corfit <- duplicateCorrelation(v, design, block=CBsamples$DamID)
...
fit <- voomLmFit(DGE, design, block=CBsamples$DamID, plot=TRUE)
fit2 <- contrasts.fit(fit,my.contrasts)
fit2 <- eBayes(fit2,robust=TRUE )
...
#save
save(RG,RGcpm,DGE,log2cpm, CBsamples,design,my.contrasts,fit,fit2, file="RNAseqDEGlist.CB.RData")
...
CBsamples$Name <- paste(CBsamples$Mouse.Number,CBsamples$Group,sep=".")
CBsamples$Name <- gsub("\\."," ", CBsamples$Name)

C <- unique(CBsamples$Name)
...
#plot MDS for group
CBsamples$Group <- as.factor(CBsamples$Group)

pdf(file = "MDSplot_group.pdf", wi = 12, he = 10, useDingbats=F)

levels(CBsamples$Group)
col.cell2 <- c("blue","orange","purple","red","pink","yellow","green","brown")[CBsamples$Group]
data.frame(CBsamples$Group,col.cell2)
...
levels(CBsamples$Sex)
col.cell2 <- c("blue","orange")[CBsamples$Sex]
data.frame(CBsamples$Sex,col.cell2)
...
levels(CBsamples$Dam.Treatment.Group)
col.cell <- c("blue","orange")[CBsamples$Dam.Treatment.Group]
data.frame(CBsamples$Dam.Treatment.Group,col.cell)
...
legend(1.5, 1,
       fill=c("blue","orange"),
       legend=levels(CBsamples$Dam.Treatment.Group),
       cex = 0.8)
# Add a title
...
#par(mfrow=c(1,2))
#plot MDS for treatment
levels(CBsamples$Pup.Treatment.Group)
col.cell <- c("blue","purple")[CBsamples$Pup.Treatment.Group]
data.frame(CBsamples$Pup.Treatment.Group,col.cell)
...
legend(1.5, 1,
       fill=c("blue","purple"),
       legend=levels(CBsamples$Pup.Treatment.Group),
       cex = 0.8)
# Add a title
...
tmp <- merge(CBsamples, size, by.x = "filenames",by.y = "files")
...
library(RColorBrewer)
n <- length(unique(CBsamples$DamID))
...
#par(mfrow=c(1,2))
#plot MDS for treatment
levels(CBsamples$DamID)
col.cell <- col_vector[CBsamples$DamID]
data.frame(CBsamples$DamID,col.cell)
...
countdf3 <-merge(countdf2,CBsamples,by="Sample.ID")
...
save(tfit,dt,SumTableOut,GeneSummaryCPM2,GeneSummary2,countdf,RG,genes,file = "RNAseqDEGlist.CB.RData")
#load("RNAseqDEGlist.CB.RData")

#######################################################################
#plot only Cerebellum_M_PolyICvsSaline_Treg_Volcano
#######################################################################
...
#volcano plot
pdf(file = paste(comp,"_Volcano.pdf", sep=""), wi = 6, he = 6, useDingbats=F)
...
group.df <- CBsamples[match(samplefiles, CBsamples$filenames),]
...
#add in litter info
tmp <- merge(annotation_col, CBsamples, by="Mouse.Number")
